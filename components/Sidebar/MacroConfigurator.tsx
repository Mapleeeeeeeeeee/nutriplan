...
const MacroConfigurator: React.FC<MacroConfiguratorProps> = ({ currentPlan, onUpdatePlan }) => {
    ...
    const updateMacroRatio = (key: keyof typeof currentPlan.macroRatio, value: string) => {
        if (value === '') {
            onUpdatePlan({
                ...currentPlan,
                macroRatio: { ...currentPlan.macroRatio, [key]: 0 }
            });
            return;
        }
    ...
    const updateCalories = (value: string) => {
        if (value === '') {
            onUpdatePlan({ ...currentPlan, targetCalories: 0 });
            return;
        }
        const val = parseInt(value);
        if (!isNaN(val)) {
            onUpdatePlan({ ...currentPlan, targetCalories: val });
        }
    };
            ...
            <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="bg-white p-1 rounded-lg border text-center shadow-sm">
                        ...
                        <NumberInput
                            value={currentPlan.macroRatio.carbs}
                            onChange={(v) => updateMacroRatio('carbs', v)}
                            className="w-full text-center font-bold text-gray-700 outline-none pb-1"
                        />
                    </div>
                    <div className="bg-white p-1 rounded-lg border text-center shadow-sm">
                        <span className="text-[10px] text-gray-400 block pt-1">蛋白質</span>
                        <NumberInput
                            value={currentPlan.macroRatio.protein}
                            onChange={(v) => updateMacroRatio('protein', v)}
                            className="w-full text-center font-bold text-gray-700 outline-none pb-1"
                        />
                    </div>
                    <div className="bg-white p-1 rounded-lg border text-center shadow-sm">
                        ...
                        <NumberInput
                            value={currentPlan.macroRatio.fat}
                            onChange={(v) => updateMacroRatio('fat', v)}
                            className="w-full text-center font-bold text-gray-700 outline-none pb-1"
                        />
                    </div>
                </div>
