...
import { FoodCategory, MenuPlan, NutritionTotals } from '../../types';
...
const PortionConfigurator: React.FC<PortionConfiguratorProps> = ({ currentPlan, dayStats, onUpdatePlan }) => {
    ...
    // Step 2 Verification - use actual stats from dayStats (updates with ABC choice)
    const actualStats = dayStats.totals;

    const calDiff = Math.abs(actualStats.calories - currentPlan.targetCalories);
    ...
    return (
        <>
            ...
            {/* Validation Box - Shows actual stats from selected foods */}
            <div className={`mt-4 p-4 rounded-xl border text-[11px] ${isDeviationHigh ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}>
                <div className="flex justify-between items-center mb-2">
                    <span className="font-bold text-gray-600">實際攝取：總熱量</span>
                    <span className={`font-black text-lg ${isDeviationHigh ? 'text-red-500' : 'text-emerald-600'}`}>{actualStats.calories.toFixed(0)}</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center pt-2 border-t border-gray-200/50">
                    <div>
                        ...
                        <div className={`font-bold ${Math.abs(actualStats.carbs - targetGrams.carbs) > 10 ? 'text-red-500' : 'text-gray-600'}`}>
                            {actualStats.carbs.toFixed(0)}g
                        </div>
                        <div className="text-[9px] text-gray-300">目標 {targetGrams.carbs.toFixed(0)}</div>
                    </div>
                    <div>
                        <div className="text-gray-400 mb-0.5">蛋白</div>
                        <div className={`font-bold ${Math.abs(actualStats.protein - targetGrams.protein) > 10 ? 'text-red-500' : 'text-gray-600'}`}>
                            {actualStats.protein.toFixed(0)}g
                        </div>
                        <div className="text-[9px] text-gray-300">目標 {targetGrams.protein.toFixed(0)}</div>
                    </div>
                    <div>
                        <div className="text-gray-400 mb-0.5">脂肪</div>
                        <div className={`font-bold ${Math.abs(actualStats.fat - targetGrams.fat) > 10 ? 'text-red-500' : 'text-gray-600'}`}>
                            {actualStats.fat.toFixed(0)}g
                        </div>
